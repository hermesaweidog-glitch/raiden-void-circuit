...
test('high-sitting passive artwork is shifted down consistently across all icon surfaces', () => {
  for (const icon of ['payload', 'harvester', 'bombcap', 'flux', 'critical', 'salvage', 'armor', 'capacitor']) {
    assert.match(css, new RegExp(`src\\$=\"/${icon}\\.webp\"`));
  }
  assert.match(css, /translateY\(3px\) scale\(\.78\)/);
  assert.match(css, /translateY\(1px\) scale\(\.86\)/);
});
...
test('rebrand keeps legacy progression storage while exposing the new public title', () => {
  assert.match(html, /相位入侵/);
  assert.match(html, /PHASE INCURSION/);
  assert.doesNotMatch(html, /RAIDEN: VOID CIRCUIT/);
  assert.match(main, /window\.phaseIncursionGame = debugApi/);
});


test('Claude entry and main menu are wired to the existing game flow', () => {
  for (const id of ['phase-title-screen','phase-menu-screen','phase-start','phase-menu-ore','phase-hangar','phase-codex','phase-max-toggle','phase-reset']) {
    assert.match(html, new RegExp(`id=["']${id}["']`));
  }
  assert.match(main, /initTitleShell/);
  assert.match(main, /onMode: mode => showLoadout\(mode\)/);
});


test('runtime XP bar is forced visible during active runs', () => {
  assert.match(css, /body\.run-active \.xp-status\{[^}]*display:grid!important/);
  assert.match(css, /body\.run-active \.xp-status \.xp-track\{[^}]*height:8px/);
});

test('upgrade choices use portrait cards with enlarged icons', () => {
